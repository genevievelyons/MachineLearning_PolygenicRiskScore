...
var_pheno = args.var_phenotype         
clumped_unclumped = args.clumped_unclumped         
rel = args.rel
...
def run_xgb(var, X_train, X_test, y_train, y_test, 
    ...
    # Save model
    if save==True:
        pickle.dump(xgb_model, open(model_weights_dir + 'xgb_{}.sav'.format(var), 'wb'))
        # Example: load the model from disk
...
########################################
# Identify alpha values for CV
########################################

# Identify alpha values for CV using XGB
fname_start = '{}_{}_SNPs_p1e-4*TOPMedTrain_Agefixed_MedicationFixedNoTCTGfix_Dedup{}Rel_genotypes_encoded_LASSO_part'.format(phenotype, clumped_unclumped, rel)
alphas = glob.glob(lasso_dir + fname_start + '*{}*'.format(race))
alphas = pd.Series([f.split('_')[-1].replace('.csv.gz','') for f in alphas], name='alpha')
alphas = np.array(alphas.drop_duplicates().sort_values())

# Split into n jobs
alphas = np.array_split(alphas, n_jobs)

# Use jobindex to identify this chunk of alphas
alphas = alphas[int(jobindex)-1]

########################################
# CV for optimal value of alpha
########################################
...
for a in alphas: 
    print("Starting alpha = {}".format(a))
    # Load Data
    X_train, X_test, y_train, y_test, pheno_train, pheno_test = load_data(var=phenotype, 
            var_pheno=var_pheno, race=race, rel=rel, lasso_lim=True, a=a,
    ...
    # Concat in PRS results, only if we want to run with PRS
    ...
    # Run LASSO
    genetic_component_train, genetic_component_test, test_rmse_lasso, test_evr_lasso = run_lasso(phenotype, 
                train_df, test_df, 
                y_train, y_test, save=False)
    ...
    # Append info
    test_rmse_lassos.append(test_rmse_lasso)
    test_evr_lassos.append(test_evr_lasso)
    mean_rmse_cv_xgbs.append(mean_rmse_cv_xgb)
    test_evr_xgbs.append(test_evr_xgb)
    num_snps.append(X_train.shape[1])
    
# Collect the results of CV
metadata = {'alpha' : alphas, 
            'test_rmse_lasso': test_rmse_lassos,
            'test_evr_lasso': test_evr_lassos,
            'mean_rmse_cv_xgb': mean_rmse_cv_xgbs,
            'test_evr_xgb': test_evr_xgbs,
            'num_snps' : num_snps}
...
# Save to CSV
fname = '{}_{}_{}_{}Rel_{}_cvresults_{}.csv'.format(phenotype, race, clumped_unclumped, rel, include_prs, jobindex)
