...
parser.add_argument('train_race', type=str, help='race the model was trained on: White, Black, HL, ALL')
...
def load_data(var, var_pheno, race, rel, lasso_lim=True, a=0, clumped_unclumped='unclumped'):
    ...
    if lasso_lim==True:
        ...
        files = glob.glob(lasso_dir + fname_start + '*{}*Lambda_{}.csv.gz'.format(train_race, a))
        if len(files) == 0:
            files = glob.glob(lasso_dir + fname_start + '*{}*Lambda_{}.csv.gz'.format('ALL', a)) 
...
#######################################################################################################
## Define function to load XGB, assess, and report out metrics ##
#######################################################################################################

def assess_xgb(var, train_race, clumped_unclumped, X_test, y_test):

    # Load XGB
    xgb_model = pickle.load(open(model_weights_dir + 'xgb_{}_{}_{}.sav'.format(var, train_race, clumped_unclumped), 'rb'))

    # Features 
    features = xgb_model.feature_names
    # Identify missing features 
    missing_features = np.setdiff1d(features, X_test.columns)
    # Append these features in, all as 0 (they are the missing races)
...
# Assess on XGB
test_evr_xgb = assess_xgb(phenotype, train_race, clumped_unclumped, test_df, y_test)

# Write out results 
fdir = ''
fname = 'assess_results_by_race_v3.txt'

file_object = open(fdir + fname, 'a')
file_object.write('{},{},{},{},{}\n'.format(phenotype,train_race, race, clumped_unclumped,test_evr_xgb))
file_object.close()
